...
__dag_properties = {'dag_id': '4', 'description': None}
__tasks = [{'task_id': 14, 'task_name': 'read_file_14', 'pipeline_options': {'config': '{"ports": {"inputs": [], "outputs": [{"id": 2, "name": "string", "type": "string", "config": {"file_path": "test_output"}, "priority": 0, "required": true, "port_status": "Port Status 1"}]}, "configuration": {"file_path": "/opt/airflow/dags/test_input.csv"}}'}, 'py_file': None}]
__dependencies = {20: [19, 21], 19: [], 21: []}
...
def _check_arg_beam(task_properties):
    params = {
        "task_id": None, "task_name": None, "type": "BeamRunPythonPipelineOperator",
        "py_file": "apache_beam.examples.wordcount",
        "py_options": ['-m'], "py_requirements": ['git+https://szainalie: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@github.com/buildwithlagrangeai/lagrange-workflow.git@develop'],
...
def _find_task_object(task_id: str, task_object: List):
...
try:
    with DAG(
            dag_id=__dag_pram['dag_id'].split(" ")[0], description=__dag_pram['description'], timetable=__dag_pram['timetable'],
